...
vehiclesrouter.get("/available", async (req, res, next) => {
  try {
    let { date } = req.query;
    // Formatear la fecha para la consulta SQL
    const result = await pool.query(
      `
        SELECT v.id, v.plate, v.color, v.cargo_capacity,
        ...
        WHERE v.id NOT IN (
            SELECT vehicle_id
            FROM routes
            WHERE date = $1 AND vehicle_id IS NOT NULL
        )
      `,
      [date]
    );
...
vehiclesrouter.delete("/:plate", async (req, res) => {
  try {
    ...
    const result = await pool.query("SELECT * FROM delete_vehicle($1)", [
      plate,
    ]);
